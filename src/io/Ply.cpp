...
namespace io
{
      ...
      const std::vector<geometry::Vec3f>& normals)
  {
    ...
    outFile.add_properties_to_element(
        "vertex", {"x", "y", "z"}, ply::Type::FLOAT32, xyz.size(),
        reinterpret_cast<const uint8_t*>(xyz.data()), ply::Type::INVALID, 0);
    ...
    if(rgb.size() > 0)
    {
      ...
      outFile.add_properties_to_element(
          "vertex", {"red", "green", "blue"}, ply::Type::UINT8, rgb.size(),
          reinterpret_cast<const uint8_t*>(rgb.data()), ply::Type::INVALID, 0);
    }
    if(normals.size() > 0)
    {
      ...
      outFile.add_properties_to_element(
          "vertex", {"nx", "ny", "nz"}, ply::Type::FLOAT32, normals.size(),
          reinterpret_cast<const uint8_t*>(normals.data()), ply::Type::INVALID, 0);
    }
      ...
      const std::vector<geometry::Vec3i>& triangles)
  {
    ...
    outFile.add_properties_to_element(
        ...
        reinterpret_cast<const uint8_t*>(rgb.data()), ply::Type::INVALID, 0);
    outFile.add_properties_to_element(
        "face", {"vertex_indices"}, ply::Type::INT32, triangles.size(),
        reinterpret_cast<const uint8_t*>(triangles.data()), ply::Type::UINT8, 3);
