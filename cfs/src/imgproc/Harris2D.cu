    ...
    const size_t outputStride)
{
    for(int i = blockIdx.x * blockDim.x + threadIdx.x; i < h; i += blockDim.x * gridDim.x)
    {
        for(int j = blockIdx.y * blockDim.y + threadIdx.y; j < w; j += blockDim.y * gridDim.y)
        {
            ...
            const float len = math::Vec2f{gx, gy}.Len();
            const float theta
                = (gx > 0.0f) ? ((gx >= 0.0f) ? std::atan(gy / gx) : (M_PI - std::atan(-gy / gx)))
                              : 0.0f;
            gradients[i * outputStride + j] = math::Vec2f{len, theta};
        }
    }
}

__global__ __launch_bounds__(tileWidth* tileHeight) static void extractEdgesKernel(
    const math::Vec2f* __restrict__ gradients,
    ...
    const size_t outputStride)
{
    //   static const std::array<math::Vec2i, 16> indices = {
    //
    //   };
    //   for(int i = blockIdx.x * blockDim.x + threadIdx.x; i < h; i += blockDim.x * gridDim.x)
    //   {
    //     for(int j = blockIdx.y * blockDim.y + threadIdx.y; j < w; j += blockDim.y * gridDim.y)
    //     {
    //       const math::Vec2f p = gradients[i * inputStride + j];
    //       const math::Vec2f neighbours[8] = {
    ...
    //           readPixel(gradients, i + 1, j + 1, w, h, inputStride, p)};
    //
    //       // TODO : look for the two nn from the direction of the gradient dependding on the
    //       orientation
    //       // id
    //
...
void Harris2D<T>::compute(const Image2D<ImageFormat::R, T>& input, const cudaStream_t& stream)
{
    ...
    extractGradientsKernel<<<dim3(16, 16), dim3(32, 32), 0, stream>>>(
        ...
        input.height(),
        input.img().stride(),
        gradients_.img().stride());
    extractEdgesKernel<<<dim3(16, 16), dim3(32, 32), 0, stream>>>(
        gradients_.img().data(),
        edges_.img().data(),
        gradients_.img().width(),
        gradients_.img().height(),
        gradients_.img().stride(),
        edges_.img().stride());
