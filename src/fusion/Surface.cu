...
void Surface::transform(const geometry::Mat4d& m, const cudaStream_t& stream)
{
...
void Surface::exportPLY(const std::string& filename, const cudaStream_t& stream)
{
  ...
  CpuPtr<geometry::Vec3i, true> trianglesCpu(triangles_.size());
