...
public:
  ...
  BaseImg(const BaseImg& cp) : width_{cp.width_}, height_{cp.height_}
  {
    ...
    gpuErrcheck(cudaMemcpy2D(
        data_, pitch_, cp.data_, cp.pitch_, width_ * sizeof(T), height_, cudaMemcpyDeviceToDevice));
  }
  ...
  void uploadFrom(const BasePtr<SrcType, SrcAlloc>& src, const cudaStream_t& stream)
  {
    ...
    gpuErrcheck(cudaMemcpy2DAsync(
        this->data(), this->pitch_, src.data(), width_ * sizeof(SrcType), width_ * sizeof(SrcType),
        height_, cudaMemcpyHostToDevice, stream));
  }
  ...
  void downloadTo(BasePtr<DstType, DstAlloc>& dst, const cudaStream_t& stream) const
  {
    ...
    gpuErrcheck(cudaMemcpy2DAsync(
        dst.data(), width_ * sizeof(DstType), this->data(), this->pitch_, width_ * sizeof(DstType),
        height_, cudaMemcpyDeviceToHost, stream));
  }
...
private:
  ...
  static void copyImages(DstType& dst, const SrcType& src, const cudaStream_t& stream)
  {
    ...
    gpuErrcheck(cudaMemcpy2DAsync(
        dst.data(), dst.pitch_, src.data(), src.pitch_,
        dst.width_ * sizeof(typename DstType::value_type), dst.height_,
