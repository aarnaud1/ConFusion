...
namespace geometry
{
  ...
  struct Vec4
  {
    ...
    ATTR_HOST_DEV_INL T Len() { return std::sqrt(x * x + y * y + z * z + w * w); }

    ATTR_HOST_DEV_INL T LenSq() const { return x * x + y * y + z * z + w * w; }

    ATTR_HOST_DEV_INL T Dist(Vec4 const &v0)
    {
    ...
    {
      return Vec4(v0) * (T(1) / std::sqrt(v0.x * v0.x + v0.y * v0.y + v0.z * v0.z + v0.w * v0.w));
    }
