    ...
    const cudaStream_t& stream)
{
  ...
  computeGradientsKernel<<<dim3(16, 16), dim3(32, 32), 0, stream>>>(
      input.img().data(), gradients.img().data(), input.width(), input.height(), input.img().stride(),
    ...
    const cudaStream_t& stream)
{
  ...
  extractEdgesKernel<<<dim3(16, 16), dim3(32, 32), 0, stream>>>(
      gradients.img().data(), edges.img().data(), gradients.width(), gradients.height(),
      gradients.img().stride(), edges.img().stride(), minThr_, maxThr_);
}
