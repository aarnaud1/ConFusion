...
class BasePtr final
{
  public:
    ...
    void copyFrom(const SrcType* src, const size_t n, const cudaStream_t& stream)
    {
        gpuErrcheck(cudaMemcpyAsync(
            ...
            src,
            n * sizeof(T),
            TransferType<memory_kind, memory_kind>::value,
            stream));
    }
    ...
    {
        gpuErrcheck(
            cudaMemcpyAsync(dst, this->data(), n * sizeof(T), cudaMemcpyDeviceToHost, stream));
    }
    ...
    template <typename SrcType, typename SrcAlloc>
    void uploadFrom(
        const BasePtr<SrcType, SrcAlloc>& src, const size_t n, const cudaStream_t& stream)
    {
        using SrcPtrType = BasePtr<SrcType, SrcAlloc>;
        static_assert(
            memory_kind == MemType::DEVICE, "Destination for uploadFrom() must be device");
    ...
    void downloadTo(BasePtr<DstType, DstAlloc>& dst, const cudaStream_t& stream) const
    {
        ...
        copyBuffers(dst, *this, this->size(), stream);
    }
    template <typename DstType, typename DstAlloc>
    void downloadTo(
        BasePtr<DstType, DstAlloc>& dst, const size_t n, const cudaStream_t& stream) const
    {
        using DstPtrType = BasePtr<DstType, DstAlloc>;
        static_assert(memory_kind == MemType::DEVICE, "Source for downloadTo() must be device");
        static_assert(
  ...
  private:
    ...
    static void copyBuffers(
        DstType& dst, const SrcType& src, const size_t n, const cudaStream_t& stream)
    {
