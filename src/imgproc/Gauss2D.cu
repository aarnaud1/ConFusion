  ...
  for(int i = blockIdx.x * blockDim.x + threadIdx.x; i < h; i += blockDim.x * gridDim.x)
  {
    for(int j = blockIdx.y * blockDim.y + threadIdx.y; j < w; j += blockDim.y * gridDim.y)
    {
      ...
      for(int ii = -int(k); ii < int(k); ++ii)
      {
        ...
        {
          if(geometry::any(
                 geometry::lessThan(geometry::Vec2i{i + ii, j + jj}, geometry::Vec2i{0, 0})))
          {
            continue;
          }
          if(geometry::any(geometry::greaterThanEqual(
                 geometry::Vec2i{i + ii, j + jj}, geometry::Vec2i{int(h), int(w)})))
          {
            continue;
          }
...
template <typename T>
__global__ static void applyKernel(
    const geometry::Vec3<T>* __restrict__ input,
    geometry::Vec3<T>* __restrict__ output,
    const double* __restrict__ kernel,
    const size_t w,
    const size_t h,
    const size_t inputStride,
    ...
    for(int j = blockIdx.y * blockDim.y + threadIdx.y; j < w; j += blockDim.y * gridDim.y)
    {
      ...
      for(int ii = -int(k); ii < int(k); ++ii)
      {
        for(int jj = -int(k); jj < int(k); ++jj)
        {
          ...
          const double kVal = kernel[ii * kStride + jj];
          acc += kVal * geometry::Vec3d(input[(i + ii) * inputStride + j + jj]);
          kSum += kVal;
        }
      }
      acc /= kSum;
      output[i * outputStride + j] = geometry::Vec3<T>(acc);
    }
  }
}
    ...
    , kernelGpu_{kernelSize_}
{
  ...
  for(size_t i = 0; i < kernelStride_; ++i)
  {
    ...
    {
      const auto u = geometry::Vec2d{double(i) - k_, double(j) - k};
    ...
    const Image2D<format, T>& input, Image2D<format, T>& output, const cudaStream_t& stream)
{
  ...
  applyKernel<<<dim3(32, 32), dim3(16, 16), 0, stream>>>(
      input.img().data(), output.img().data(), kernelGpu_, w, h, input.img().stride(),
      output.img().stride(), k_, kernelStride_);
}
